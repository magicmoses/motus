...
async function PaperFeed({
  ...
  // Count from enrichments table — simpler, avoids !inner join count inconsistencies
  // Note: search filter (on papers.title) is not applied to count — minor limitation
  let countQ = supabase
    .from('enrichments')
  ...
  // Data from papers with inner join — apply all filters
  let dataQ = supabase
    .from('papers')
